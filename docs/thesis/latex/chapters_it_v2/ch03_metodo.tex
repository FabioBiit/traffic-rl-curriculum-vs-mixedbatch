...
è successo. La regola è stata messa alla prova durante lo sviluppo, dove
...
numeri assoluti più bassi. È un pregio, non un difetto: lontano dal
pavimento e dal soffitto la metrica conserva spazio per muoversi in
...
La varianza da run a run in questo ambiente è grande e il budget di
calcolo non permetteva di replicare ogni modifica candidata. Il
protocollo adottato in risposta ha quattro regole. \textbf{(1) Una
manopola alla volta}: ogni candidato è un diff stretto e isolato
comportamentalmente --- un singolo termine di reward, una singola
soglia, un singolo gruppo di osservazioni. \textbf{(2) A/B appaiato a
budget diagnostico fisso}: ogni candidato gira per $3\times10^{5}$ passi
contro la baseline corrente del trunk, con lo stesso seed e --- grazie al
...
dissentono, e alcune manopole che hanno fallito il gate sono state
consapevolmente mantenute, con l'ipotesi registrata come falsificata,
...
seed da una \texttt{SeedSequence} esplicita (\cref{sec:mondo}); la
lezione è che l'appaiamento va costruito e verificato, non assunto.
