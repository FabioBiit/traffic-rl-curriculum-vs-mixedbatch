...
\paragraph{Erosione entro la run.}
...
$-18.6$\,pp (MLP-batch). Due meccanismi candidati sono compatibili
con i dati e non separabili a $n{=}1$: la co-adattazione tra classi,
il comportamento dei pedoni degrada in parallelo e alimenta il
...
architetturale e si evince che sotto il critic GNN l'erosione è dimezzata.
...
Regimi e critic cambiano \emph{quanto} una policy si blocchi, non il
\emph{come}. Coerentemente, il profilo lento-e-in-stallo
...
episodi con target di \SI{80}{m} e traffico come in addestramento. La
\cref{fig:town05} riassume il quadro per cella.

\begin{figure}[t]
  \centering
  \captionsetup{width=0.72\textwidth}  % la figura e' larga 0.70\textwidth
...
curriculum si trasferisce per intero su strade mai viste.
...
% ------------------------------------------------------------
\section{Asse architetturale: critic MLP e GNN}
\label{sec:asse-arch}
...
\begin{table}[t]
  ...
  \label{tab:ipotesi}
  \begin{tabular}{@{}L{2.0cm}L{4.3cm}L{2.3cm}L{4.2cm}@{}}
    \toprule
...
La \cref{tab:ipotesi} confronta ciascuna predizione con l'evidenza
raccolta. Due cose sopravvivono a ogni cella, ogni strato ed entrambe le
...
più in fretta all'inizio (però a parità di budget ha prodotto una policy più efficente 
e con circa 1h in meno di addestramento totale), e la copertura più ampia del batch non ha
comprato robustezza.
